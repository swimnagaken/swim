            ...
            <div class="flex space-x-1">
                ...
                <a 
                    href="practice.php?<?php 
                        ...
                        if ($action === 'search') $params['action'] = 'search';
...
?>querySelectorAll('.set-item');
    ...
    function updateSetIndexes() {
        const container = document.getElementById('sets-container');
        const setItems = container.            <div class="flex justify-between items-center mb-3">
                <h4 class="font-medium">セット {index}</h4>
                <button type="button" class="text-red-600 hover:text-red-800 remove-set">
                    <i class="fas fa-times"></i> 削除
                </button>
                    ...
                    <div class="flex space-x-2">
                        <div class="w-1/2">
                            <select
                                ...
                                class="w-full border border-gray-300 rounded-md px-3 py-2"
                            >
                                <option value="0" <?php echo $duration_hours === 0 ? 'selected' : ''; ?>>0時間</option>
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                <option value="<?php echo $i; ?>" <?php<?php
// 設定ファイルの読み込み
